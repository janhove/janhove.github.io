...
knitr::opts_chunk$set(error = TRUE, fig.width = 9)
```
...
```{r, echo = FALSE, eval = TRUE, results = 'hide', message = FALSE, warning = FALSE, fig.height = 6}
# Load packages
library("plyr")          # to reformat data; use 'install.packages("plyr")' if not available
library("ggplot2")       # for plotting
library("gridExtra")     # for creating side-by-side plots in ggplot2

# Read in data
dat <- read.csv("http://janhove.github.io/datasets/correspondencerules.csv")

# Rename levels
levels(dat$LearningCondition) <- c("<ij> participants", "<oe> participants")


# Divide data into training and test part
dat_post <- droplevels(subset(dat, Block!="Training"))

# Restrict analysis to critical items (i.e. oe and ij cognates)
dat_post_critical <- droplevels(subset(dat_post, Category %in% c("oe cognate", "ij cognate")))

# Figure 1
# Step 1: summarise percentages per category per subject in a slightly different way
perSubjectCorrect <- ddply(dat_post_critical, 
                           .(Subject, Category, LearningCondition), 
                           summarise,
                           Correct = 100*table(Correct)[["yes"]] / sum(table(Correct)),
                           CorrectVowel = 100*table(CorrectVowel)[["yes"]] / sum(table(CorrectVowel)))
levels(perSubjectCorrect$Category) <- c("<ij> cognates", "<oe> cognates")
levels(perSubjectCorrect$LearningCondition) <- c("<ij>", "<oe>")

perSubjectCorrectOe <- droplevels(subset(perSubjectCorrect, Category == "<oe> cognates"))

# Step 2: draw plot

gp <- ggplot(perSubjectCorrectOe,
...
gp1 <- gp + stat_summary(fun.y = mean, geom = "bar", position = "dodge", mapping = aes(fill = LearningCondition), color = "black", size = 1) +
  guides(fill = FALSE, colour = FALSE) +
  xlab("Learning condition") + ylab("Percentage correct translations")

# lowCI en HiCI
lowCI <- function(x) {return(mean(x) + qt(0.025, length(x)) * sqrt(var(x)/length(x)))}
hiCI <- function(x) {return(mean(x) + qt(0.975, length(x)) * sqrt(var(x)/length(x)))}

gp2 <- gp + 
  stat_summary(fun.y = mean, geom = "bar", position = "dodge", mapping = aes(fill = LearningCondition), color = "black", size = 0.8) +
  stat_summary(fun.y = mean, fun.ymin = lowCI, fun.ymax = hiCI, geom="errorbar", position = "dodge", colour = "black", width = 0.3) +
  xlab("Learning condition") + ylab("Percentage correct translations")

gp3 <- gp + 
  stat_summary(fun.y = mean, geom = "bar", position = "dodge", mapping = aes(fill = LearningCondition), color = "black", size = 0.8) +
  stat_summary(fun.y = mean, fun.ymin = mean, fun.ymax = hiCI, geom="errorbar", position = "dodge", colour = "black", width = 0.3) +
  xlab("Learning condition") + ylab("Percentage correct translations")
...
```{r, echo = FALSE, eval = TRUE, message = FALSE, warning = FALSE, fig.height = 6}
gp4 <- gp + geom_point(position = position_jitter(h = 0, w = 0.2), size = 1.5, aes(color = LearningCondition))  +
  xlab("Learning condition") + ylab("Percentage correct translations")


gp5 <- gp + geom_boxplot(aes(color = LearningCondition))  +
  xlab("Learning condition") + ylab("Percentage correct translations")

gp6 <- gp + geom_boxplot(outlier.shape = NA, aes(color = LearningCondition)) +
  geom_point(position = position_jitter(h=0, w=0.2), size=1.5, aes(color = LearningCondition))  +
  xlab("Learning condition") + ylab("Percentage correct translations")
...
```{r, echo = FALSE, eval = TRUE, warning = FALSE, message = FALSE, fig.width = 4, fig.height = 4}
ladina <- read.csv("http://janhove.github.io/datasets/ladina.csv")
# Vpn-ID kreieren; Vpn1 bei Franz != Vpn1 bei Germ
ladina$Vpn2 <- factor(paste(ladina$L1, ladina$ID, ""))
ladina$Survey <- factor(ifelse(ladina$L1 == "French", "French survey", "German survey"))

# Nur "unknown" und French
ladina2 <- droplevels(subset(ladina, truthfulness=="notknown" & Survey == "French survey" & accent %in% c("French", "German")))

levels(ladina2$accent) <- c("Condition 1", "Condition 2")

lad1 <- ggplot(ladina2,
             aes(x = accent,
                 y = score,
                 colour = accent)) +
  guides(fill = FALSE, colour = FALSE) + geom_boxplot() +
...
```{r, echo = FALSE, eval = TRUE, warning = FALSE, message = FALSE, fig.width = 4, fig.height = 3}
lad2 <- ggplot(ladina2,
            aes(score)) +
  geom_histogram(aes(fill = accent)) +
  facet_wrap(~ accent, ncol=2) + # ersetze ncol = 1 durch ncol = 2 für nebeneinander
  theme_bw() +
  xlab("Score") +
  ylab("Frequency") +
  guides(fill = FALSE)
...
```{r, echo = FALSE, eval = TRUE, warning = FALSE, message = FALSE, fig.width = 11, fig.height = 11}
lad3 <- ggplot(ladina2,
             aes(x = accent,
                 y = score,
                 colour = accent)) +
  ...
  geom_point(position = position_jitter(h = 0, w = 0.2), size = 1.5, aes(color = accent), alpha = 0.2) +
  xlab("Condition") + ylab("Score") 

lad4 <- ggplot(ladina2,
            aes(score, ..density.., colour = accent)) +
...
### Code and links
I didn't want to clutter this post with code block after code block. If you're interested, you can download the R (and Markdown) code here (open in RStudio).
