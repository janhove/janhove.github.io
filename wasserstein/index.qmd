...
don't require the predictors $x_1, \dots, x_n$ to be represented explicitly
(for instance, as numerical vectors)
and rely only on a notion of the pairwise **similarities** between 
the predictors.
...
summation or pointwise multiplication.
...
Such kernels are known as **stationary kernels**.
...
the inputs are represented as numeric vectors (i.e., vectors in $\mathbb{R}^d$).
...
::: {.column width="40%"}
![Word meanings can be represented as vectors in $\mathbb{R}^d$, so document contents can be thought of as point clouds in $\mathbb{R}^d$.](wordembeddings.png)
:::

::: {.column width="35%"}
![The lexical frequency profile of a text can be represented as the empirical distribution of the Zipf values (~ frequencies) of the word occurring in the text.](frequencyprofile.svg)
:::
::::
...
One alternative is to either treat the objects as **probability distributions**
or to encode them as probability distributions
...
distribution.
For graphs, it takes a bit more work.
...
Wasserstein distances between univariate distributions, Wasserstein distances 
...
One possible disadvantage is that two distributions may look completely
different from one another yet have the same marginal distributions:
...
2. It is a valid distance between probability distributions with finite second moment. (This includes all empirical distributions.)
3. This distance can be slotted into the Gaussian RBF, power exponential and 
Matérn functions to give a valid kernel.
4. It takes into account the entire distributions, not just the marginals.
...
3. You can't really draw input-output plots like in the applet above...^[In principle, it should be possible to apply multidimensional scaling in order to draw some kind of plot.]
...
* E.g., for each speaker, measure the degree of overlap between the cloud 
