...
date: "2016-11-21"
categories: [effect sizes, graphs, correlational studies, non-linearities, R]
---

In this post, I want to, first, help you to improve your 
intuition of what data patterns correlation coefficients can represent 
...
For instance, say you've asked 30 people about their weight and body height and 
plot these 30 (weight, height)-pairs in a scatterplot.
...
To illustrate this, 
I wrote an `R` function, [`plot_r()`](https://janhove.github.io/cannonball/reference/plot_r.html), 
...
<i>Install the `cannonball` package, which contains the `plot_r()` function.</i>
Then, in `R`, you can draw scatterplots corresponding to a correlation coefficient of _r_ = 0.5 
and 50 observations like this. 
(You'll end up with a different set of scatterplots as the scatterplots are created randomly.)

**Update (2023-08-08):** The `plot_r()` function is now also part of the `cannonball` package
for R, which you can download from [GitHub](https://github.com/janhove/cannonball).

```{r, echo = FALSE}
set.seed(20-11-2016)
```

```{r, fig.width = 8, fig.height = 8, echo = TRUE, warning = FALSE, message = FALSE}
...
(see [reason no. 2](../2015-02-05-standardised-vs-unstandardised-es) why I don't particularly like correlation coefficients to begin with).
...
```{r, fig.width = 8, fig.height = 8, echo = TRUE, warning = FALSE, message = FALSE}
cannonball::plot_r(r = 0.1, n = 50)
```

### A strong negative correlation: _r_ = -0.9
```{r, fig.width = 8, fig.height = 8, echo = TRUE, warning = FALSE, message = FALSE}
cannonball::plot_r(r = -0.9, n = 50)
```
...
cannonball::plot_r(r = 0.4, n = 10, showdata = 11)
# Show data for all plots (not run)
cannonball::plot_r(r = 0.8, n = 15, showdata = "all")
```
