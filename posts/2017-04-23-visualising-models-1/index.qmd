...
(Check out the 
[tutorial on drawing line charts](../2016-06-13-drawing-a-linechart)
...
```{r, fig.width = 4.5, fig.height = 2.8, echo = FALSE}
nd2_levb <- expand.grid(
  # A sequence of c.Lev values
  # from the sample minimum
  # through the sample maximum
  # in steps of 0.05
...
nd2_levb$Probability <- predict(mod2, nd2_levb,
                               type = "response")

# De-centre c.Lev
nd2_levb$Levenshtein <- nd2_levb$c.Lev + mean(ex2$MinLevGermanic)

# Relabel clog.Freq
nd2_levb$clog.Freq <- factor(nd2_levb$clog.Freq)
