...
date: "2019-04-11"
categories: [simplicity, graphs, non-linearities, assumptions]
---
...
#| fig.cap: "**Figure 1.** Illustration of the linearity, homoskedasticity and normality assumptions for a linear regression model with one continuous predictor. The blue regression lines connects the fitted y-values at each x-value and is assumed to be straight. The red density curves visualise that the model assumes the y-data at each x-value to be normally distributed with the same variance at different x-values."
set.seed(1)
x <- runif(150)
dat <- data.frame(x = x,
                  y = 0.4 + 4*x + rnorm(length(x), sd = 0.5))
...
par(cex = 1.2, bty = "n",
    ...
    tcl = -0.2,
    las = 1)
plot(dat, pch = 21, col = "black", bg = "grey",
     xlab = "predictor", ylab = "outcome",
     xlim = c(0, 1),
     ylim = c(min(df1$yval),
              max(df3$yval)))
with(df1,lines(xval,yval, col = "#E41A1C"))
with(df2,lines(xval,yval, col = "#E41A1C"))
with(df3,lines(xval,yval, col = "#E41A1C"))
segments(x0 = 0.2, x1 = 0.2+0.09973557,
         y0 = 0.4+4*0.2, col = "#E41A1C")
segments(x0 = 0.5, x1 = 0.5+0.09973557,
         y0 = 0.4+4*0.5, col = "#E41A1C")
segments(x0 = 0.8, x1 = 0.8+0.09973557,
         y0 = 0.4+4*0.8, col = "#E41A1C")
abline(v = 0.2, lty = 2, col = "grey")
abline(v = 0.5, lty = 2, col = "grey")
abline(v = 0.8, lty = 2, col = "grey")
abline(0.4, 4, col = "darkblue")
```
...
par(cex = 1.2, bty = "n",
    ...
    las = 1,
    cex.main = 1)
plot(x = d$Age, y = d$Spoken, pch = 21, 
     col = "black", bg = "grey",
     xlab = "age (years)", ylab = "correct translations\n(spoken words)",
     main = "outcome vs. predictor")
m <- lm(Spoken ~ Age, data = d)
abline(m, col = "darkblue")
d$resid <- resid(m)
d$fitted <- fitted(m)
plot(x = d$fitted, y = d$resid, pch = 21, 
     col = "black", bg = "grey",
     xlab = "fitted values", ylab = "residuals",
     main = "diagnostic plot")
lines(lowess(x = d$fitted, y = d$resid, f = 0.3), col = "darkblue")
...
par(cex = 1.2, bty = "n",
    ...
    las = 1,
    cex.main = 1)
d$n.Condition <- ifelse(d$Condition == "two6", 2, 1)
#d$y <- sqrt(d$RollsImagined)
d$y <- d$RollsImagined
plot(x = jitter(d$n.Condition), y = d$y,
     xlim = c(0.5, 2.5),
     xaxt = "n",
     pch = 21, 
     col = "darkgrey", bg = "lightgrey",
     xlab = "predictor",
     ylab = "outcome",
     main = "outcome vs. predictor")
...
m <- lm(y ~ n.Condition, d)
d$fitted <- fitted(m)
d$resid <- resid(m)

# qqnorm(d$resid, pch = 21, bg = "grey", col = "black",
#        main = "diagnostic plot")
# qqline(d$resid)
hist(d$resid, col = "grey", main = "diagnostic plot",
     xlab = "residuals", breaks = 12)
```
...
```{r, echo = FALSE, eval = TRUE, fig.height = 3, fig.width = 8}
#| fig.cap: "**Figure 4.** The spread around the regression line increases with the predictor variable. (simulated data)"
set.seed(12345)
d <- cannonball::plot_r(n = 80, r = 0.3, showdata = 7, plot = FALSE)
...
d$fitted <- fitted(m)
d$resid <- resid(m)
d$sqrt.abs.resid <- sqrt(abs(d$resid))

# qqnorm(d$resid, pch = 21, bg = "grey", col = "black",
#        main = "diagnostic plot")
# qqline(d$resid)
plot(x = d$fitted, y = d$sqrt.abs.resid,
     pch = 21, 
     col = "black", bg = "grey",
     xlab = "fitted values",
     ylab = expression(sqrt(abs("residual"))),
     main = "diagnostic plot")
lines(lowess(x = d$fitted, y = d$sqrt.abs.resid), col = "darkblue")
```
...
```{r, echo = FALSE, eval = TRUE, fig.height = 3, fig.width = 8}
#| fig.cap: "**Figure 5.** The scatter is considerably greater in Group 2 than it is in Group 1. (simulated data)"
set.seed(12345)
x <- c(rep(1, 10), rep(2, 19))
y <- 4+0.4*x
error <- c(rnorm(10, sd = 1), rnorm(19, sd = 4))
y <- y + error
d <- data.frame(x, y)
...
boxplot(y ~ x, data = d, boxwex = 0.5, xaxt = "n",
        main = "outcome vs. predictor")

points(x = jitter(d$x), y = d$y,
     xlim = c(0.5, 2.5),
     pch = 21, 
     col = "darkgrey", bg = "lightgrey")
axis(side = 1, at = c(1, 2),
...
Klein, Richard A., Kate A. Ratliff, Michelangelo Vianello, Reginald B. Adams Jr., Štěpán Bahník, Michael J. Bernstein, Konrad Bocian et al. 2014. [Investigating variation in replicability: A 'many labs' replication project.](http://dx.doi.org/10.1027/1864-9335/a000178) *Social Psychology* 45(3). 142--152.
