...
[**bootstrapping**](https://books.google.ch/books/about/An_Introduction_to_the_Bootstrap.html?id=gLlpIUxRntoC)
...
```{r, fig.width = 4, fig.height = 2.5, dpi = 100, echo = FALSE, warning = FALSE, message = FALSE}
#| fig.cap: "The distribution of 159 _WST_ (German vocabulary test) scores. Disregarding the bottom 10% of this sample (16 values), its mean is 32.7."
# Draw histogram
ggplot(dat,
       aes(x = WST)) +
...
# Generate 20000 bootstrapped samples and plot distribution
trimmed.means <- replicate(20000,
                           {
...
hist_trimmedmeans <- ggplot(data.frame(TrimmedMeans = trimmed.means),
  ...
  ylab("Count") +
  ggtitle("Distribution of trimmed WST means\nin 20,000 bootstrap samples")

(plot_spacer() + hist_mother + plot_spacer()) /
(hist_child1   + hist_child2 + hist_child3) /
(plot_spacer() + hist_trimmedmeans + plot_spacer())
```
...
* when you don't have an analytic means of doing so (e.g., based on a theoretical distribution such as the _t_-distribution for sample means)  
...
We could use the regression model's output to construct a 90% confidence interval using the appropriate _t_-distribution, but for the sake of the argument, let's pretend that option is off the table.
...
but rather than computing the trimmed means for 20,000 new samples,
...
4. Perform steps 1--3 20,000 times.

```{r, echo = FALSE}
npb.coef <- replicate(20000,
                       {
                         # sample with replacement
                         sample_n(dat, 159, replace = TRUE) |> 
                           # fit model on bootstrap sample
                           lm(TotCorrect ~ z.WST + z.Raven + z.English.Cloze, data = _) |>
...
```{r, echo = FALSE}
# Simulate 20000 new datasets from model
newdata <- simulate(mod, nsim = 20000)
fit.lm <- function(x) {
...
6. Perform steps 3--5 20,000 times. 

```{r, echo = FALSE}
spb.coef <- replicate(20000,
                       {
...
```{r, fig.width = 6, fig.height = 2.5, echo = FALSE, warning = FALSE, message = FALSE}
ggplot(data.frame(x = c(npb.coef, pb.coef, spb.coef),
                  Method = c(rep("nonparametric", 20000),
                             rep("parametric", 20000),
                             rep("semiparametric", 20000))), 
       aes(x)) + 
...
we pick the 5th and 95th percentiles of the bootstrapped estimates.
The resulting intervals are plotted below alongside the 90% confidence interval constructed from the model summary.
In this case, the confidence interval obtained by nonparametric bootstrapping is a bit wider.

```{r, fig.width = 4, fig.height = 2.5, dpi = 100, echo = FALSE, warning = FALSE, message = FALSE}
df <- data.frame(Method = c("classic (t-distribution)", "nonparametric", "parametric", "semiparametric"),
                 Low = c(0.3980439, quantile(npb.coef, .05), quantile(pb.coef, .05), quantile(spb.coef, .05)),
                 High = c(1.901509, quantile(npb.coef, .95), quantile(pb.coef, .95), quantile(spb.coef, .95)))
ggplot(df,
       aes(x = Method,
           ymax = High,
           ymin = Low)) +
...
5. Perform steps 2--4 20,000 times.
6. See how often the estimated `z.WST` coefficients in the 20,000 samples generated under the null hypothesis are larger in magnitude than the estimated `z.WST` coefficient in the original sample. This is the _p_-value. 
...
# Simulate 20000 new datasets from model
newdata <- simulate(mod0, nsim = 20000)
fit.lm <- function(x) {
   lm(x ~ dat$z.WST + dat$z.Raven + dat$z.English.Cloze) |> coef() |> extract("dat$z.WST")
}
# Fit original model on 20,000 columns
pb.coef <- apply(newdata, 2, function(x) fit.lm(x))
```
...
7. Perform steps 4--6 20,000 times.
8. See how often the estimated `z.WST` coefficients in the 20,000 samples generated under the null hypothesis are larger in magnitude than the estimated `z.WST` coefficient in the original sample. This is the _p_-value.
...
The _p_-value obtained by parametric bootstrapping is `r mean(abs(pb.coef) > abs(mod$coef[2])) |> signif(3)` (i.e., `r sum(abs(pb.coef) > abs(mod$coef[2]))` out of 20,000 estimated `z.WST` coefficients have absolute values larger than 1.15),
