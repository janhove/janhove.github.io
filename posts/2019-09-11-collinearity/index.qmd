...
date: "2019-09-11"
categories: [R, multiple regression, assumptions, collinearity]
---

::: {.callout-important}
A slightly reworked version of this blog post was published in _Meta-Psychology_:
...
# Generate datasets
generate_dataset <- function(n_sample = 50, rho = 0.5, 
                             coefs = c(0.4, 1.9),
                             sd_error = 3.5) {
  ...
  data.frame(predictor1 = predictors[, 1],
             predictor2 = predictors[, 2],
             outcome = outcome)
}

set.seed(2019-09-10)
strong <- generate_dataset(rho = 0.98)

set.seed(2019-09-10)
weak <- generate_dataset(rho = 0.50)

set.seed(2019-09-10)
none <- generate_dataset(rho = 0.00)
...
nonlinear$outcome <- 0.4*nonlinear$predictor1 + 1.9*nonlinear$predictor2 +
  rnorm(50, sd = 3.5)
```

```{r, echo = FALSE, eval = FALSE}
write_csv(strong, "strong_collinearity.csv")
write_csv(weak, "weak_collinearity.csv")
write_csv(none, "no_collinearity.csv")
write_csv(nonlinear, "nonlinearity.csv")
```
...
```{r, fig.width = 9, fig.height = 7.5, echo = FALSE}
#| fig.cap: "**Figure 1.** A dataset with a strong degree of collinearity between the two predictors (r = 0.98)."
...
```{r, fig.width = 9, fig.height = 7.5, echo = FALSE}
#| fig.cap: "**Figure 2.** A dataset with a weak degree of collinearity between the two predictors (r = 0.50)."
scatterplot_matrix(weak[, c(3, 1, 2)])
```

```{r, fig.width = 9, fig.height = 7.5, echo = FALSE}
#| fig.cap: "**Figure 3.** A dataset in which the two predictors are entirely orthogonal and unrelated (r = 0.00)."
scatterplot_matrix(none[, c(3, 1, 2)])
```

```{r, fig.width = 9, fig.height = 7.5, echo = FALSE}
#| fig.cap: "**Figure 4.** A dataset with two orthogonal (r = 0.00) but perfectly related predictors: `predictor1` is a sinusoid transformation of `predictor2`. In other words, you can predict `predictor1` perfectly if you know `predictor2`."
...
#| fig.cap: "**Figure 5.** Estimated coefficients and their 90% confidence intervals for the models fitted to the four datasets."
strong_out <- tidy(strong.lm, conf.int = TRUE,
                   conf.level = 0.90) |> 
  mutate(dataset = "strong")
weak_out <- tidy(weak.lm, conf.int = TRUE,
                 conf.level = 0.90) |> 
  mutate(dataset = "weak")
none_out <- tidy(none.lm, conf.int = TRUE,
                 conf.level = 0.90) |> 
  mutate(dataset = "none")
nonlinear_out <- tidy(nonlinear.lm, conf.int = TRUE,
                      conf.level = 0.90) |> 
...
outputs |> 
  ggplot(aes(x = factor(dataset, levels = c("nonlinear", "none", "weak", "strong")),
             y = estimate,
             ymin = conf.low,
             ymax = conf.high)) +
  geom_pointrange() +
...
map_dfr(list(strong.lm, weak.lm, none.lm, nonlinear.lm), glance) |> 
  ...
  knitr::kable("html",
               caption = "Table 1. R²- and p-values for the overall model fit for the multiple regression models on the four datasets. Even though neither predictor has a significant estimated coefficient in the 'strong' dataset (shown in Figure 1), the overall fit is highly significant.") |> 
...
```{r, echo = FALSE, fig.width = 9, fig.height = 4.5}
#| fig.cap: "**Figure 8.** Dropping a collinear predictor biases the estimate for the predictor retained as well as its meaning."
ggplot(all_data,
...
#| fig.cap: "**Figure 9.** Ridge regression is a form of biased estimation, so naturally the estimates it yields are biased."
ggplot(all_data,
...
O'Brien, Robert M. 2007. [A caution regarding rules of thumb of variance inflation factors.](https://doi.org/10.1007/s11135-006-9018-6) *Quality & Quantity* 41. 673-690.

Vanhove, Jan, Audrey Bonvin, Amelia Lambelet and Raphael Berthele. 2019. [Predicting perceptions of the lexical richness of short French, German, and Portuguese texts using text-based indices.](https://doi.org/10.17239/jowr-2019.10.03.04) *Journal of Writing Research* 10(3). 499-525.

Wurm, Lee H. and Sebastiano A. Fisicaro. 2014. [What residualizing predictors in regression analyses does (and what it does not do).](https://doi.org/10.1016/j.jml.2013.12.003) *Journal of Memory and Language* 72. 37-48.

York, Richard. 2012. [Residualization is not the answer: Rethinking how to address multicollinearity.](https://www.ncbi.nlm.nih.gov/pubmed/23017962) *Social Science Research* 41. 1379-1386.
...
## Software versions
Please note that I reran the code on this page on 20 August 2026.
