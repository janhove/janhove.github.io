...
date: "2015-07-17"
categories: [power, effect sizes, logistic regression, R]
---
...
My previous discussions of the usefulness of including covariates when analysing randomised experiments 
([here](../2014-08-14-pretest-posttest-ancova) and 
[here](../2014-11-18-neglected-covariates)) dealt with the case in which the dependent variable is continuous and could be analysed in a linear regression model.
...
The set-up of the simulation is based on the one reported in my 
[first blog post](../2014-11-18-neglected-covariates).
...
<del>10,000</del> (update (2023-08-27): 3,000) datasets were generated that were analysed in two logistic models: one without and one with the covariate.
...
**Update (2023-08-27):** I've run the simulation again but with only 3,000
datasets per parameter combination. The plots from the original blog post
were redrawn using `ggplot2` instead of base R.
...
#| fig.cap: "**Figure 1** shows the models' mean estimation of the treatment effect according to the strength of the relationship between the dependent variable and the covariate and the standard deviation of the residual variability in the latent skill. The underlying treatment effect was fixed at `b.condition = 1.6` (dashed line). _Red:_ the estimate for the logistic model without the covariate. _Blue:_ the estimate for the logistic model with the covariate."
#| fig.width: 9
#| fig.height: 9
library(tidyverse)
d <- read_csv("simulatedResults2.csv") |> 
  pivot_longer(cols = c("slope.nocov", "slope.cov"),
               names_to = "adjusted",
               values_to = "estimate") |> 
  mutate(b.covariate = paste0("b.covariate = ", b.covariate))

ggplot(d,
       aes(x = sd.lat, y = estimate,
           colour = rev(adjusted))) +
  geom_line() +
  facet_wrap(~ b.covariate) +
  guides(colour = "none") +
  xlab("unexplained error in latent skill\n(sd.lat)") +
...
```{r, echo = FALSE, warning = FALSE, message = FALSE}
#| fig.cap: "**Figure 2** shows the models' mean standard error of the treatment effect according to the strength of the relationship between the dependent variable and the covariate and the standard deviation of the residual variability in the latent skill. The underlying treatment effect was fixed at `b.condition = 1.6`. _Red:_ the estimate for the logistic model without the covariate. _Blue:_ the estimate for the logistic model with the covariate."
#| fig.width: 9
#| fig.height: 9
library(tidyverse)
d <- read_csv("simulatedResults2.csv") |> 
  pivot_longer(cols = c("se.nocov", "se.cov"),
               names_to = "adjusted",
               values_to = "estimate") |> 
  mutate(b.covariate = paste0("b.covariate = ", b.covariate))
...
```{r, echo = FALSE, warning = FALSE, message = FALSE}
#| fig.cap: "**Figure 3** shows the models' power for rejecting the null hypothesis of 'no treatment' according to the strength of the relationship between the dependent variable and the covariate and the underlying treatment effect. The residual variability in the latent skill was fixed at `sd.lat = 1`. _Red:_ the estimate for the logistic model without the covariate. _Blue:_ the estimate for the logistic model with the covariate."
#| fig.width: 9
#| fig.height: 9
library(tidyverse)
d <- read_csv("simulatedResults.csv") |> 
  pivot_longer(cols = c("power.x2.nocov", "power.x2.cov"),
               names_to = "adjusted",
               values_to = "estimate") |> 
  mutate(b.covariate = paste0("b.covariate = ", b.covariate))

ggplot(d,
       aes(x = b.condition, y = estimate,
           colour = rev(adjusted))) +
  geom_line() +
  facet_wrap(~ b.covariate) +
  guides(colour = "none") +
  xlab("underlying treatment effect\n(b.condition)") +
  ylab("power")
```
...
```{r, echo = FALSE, warning = FALSE, message = FALSE}
#| fig.cap: "**Figure 4** shows the models' power for rejecting the null hypothesis of 'no treatment' according to the strength of the relationship between the dependent variable and the covariate and the standard deviation of the residual variability in the latent skill. The underlying treatment effect was fixed at `b.condition = 1.6`. _Red:_ the estimate for the logistic model without the covariate. _Blue:_ the estimate for the logistic model with the covariate."
#| fig.width: 9
#| fig.height: 9
library(tidyverse)
d <- read_csv("simulatedResults2.csv") |> 
...
`replicate.logistic` is then run <del>10,000</del> (update: 3,000) times for a combination of `b.covariate` and `b.condition` values.
