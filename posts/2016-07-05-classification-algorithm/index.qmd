...
date: "2016-07-05"
categories: [R, machine learning, random forests]
---

I'd promised to write another installment on drawing 
[graphs](../2016-06-21-drawing-a-boxplot),
but instead I'm going to write about something that 
I had to exclude, for reasons of space, 
from a recently published [book chapter](https://folia.unifr.ch/unifr/documents/304945) 
...
The procedure I've just sketched is pretty common but it's **statistically naïve**.
...
as it's sometimes implied that fewer L2 learners would've qualified as nativelike 
if only more and more reliable data were available.
This may well be true.
But the other side of the coin is rarely considered: 
not all L1 speakers may pass the nativelikeness criterion either!
...
This simulation obviously represent a huge simplification; life would be easier if people 
[put their data online](../2015-12-14-perks-data-sharing).)
...
**Update (2023-08-08):** I've reran this code using newer software versions.
I now also rely on the `tidyverse` suite of packages instead of on
the individual packages that later went on to form the `tidyverse` suite.
...
I won't provide a detailed introduction---Kuhn & Johnson's 
[_Applied Predictive Modeling_](http://appliedpredictivemodeling.com/) seems 
excellent---but I'll just illustrate one such classification algorithm, **random forests**.
...
an estimated 16.1% of observations will be misclassified by this algorithm.
...
**But it also misclassifies 7 of the 15 L1 speakers as L2 speakers.**
...
it performs much better on new L1 data, classifying 77% of new L1 speakers as L1 speakers.
...
## Conclusions
a. By using common definitions of nativelikeness criteria,
L2 acquisition studies are likely to stack the odds against findings of nativelikeness and 
yield generally uninterpretable results.

b. Random forests and other classification algorithms will yield
...
c. You're unlikely to learn about such algorithms in an introductory course to statistics,
...
n.L1 <- 15
min.L1 <- c(11.7, 10.7, 14.6, -13.0, 5.4, 17.9, -11.53, 12, 57, 58, 7160, 30, 33, 33)
max.L1 <- c(22.7, 21.8, 25.5, 27.8, 27.5, 33.3, -5.06, 18, 78, 78, 8888, 41, 48, 46)
mean.L1 <- c(16.9, 15.7, 18.3, 7.2, 15.3, 24.6, -7.46, 16, 69, 70, 7729, 36, 43, 39)
sd.L1 <- abs(min.L1-max.L1)/4
for (i in 1:length(min.L1)) {
  nam <- paste("Pred", i, sep = "")
  assign(nam, round(rnorm(n.L1, mean.L1[i], sd.L1[i])))
}
dat <- as.data.frame(cbind(Pred1, Pred2, Pred3, Pred4, Pred5,
                     Pred6, Pred7, Pred8, Pred9, Pred10,
                     Pred11, Pred12, Pred13, Pred14,
                     Class = rep("L1 speaker", n.L1)))
n.L2 <- 41
for (i in 1:length(min.L1)) {
  nam <- paste("Pred", i, sep = "")
  assign(nam, round(rnorm(n.L2, 0.9*mean.L1[i], 1.1*sd.L1[i])))
}
dat2 <- as.data.frame(cbind(Pred1, Pred2, Pred3, Pred4, Pred5,
                     Pred6, Pred7, Pred8, Pred9, Pred10,
                     Pred11, Pred12, Pred13, Pred14,
                     Class = rep("L2 speaker", n.L2)))
dat.tot <- rbind(dat, dat2)
write.csv(dat.tot, "dat_tot.csv", row.names = FALSE, quote = FALSE)
dat.tot <- read.csv("dat_tot.csv")
```
