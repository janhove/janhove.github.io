...
plot.r.fnc <- function(r = 0.6, n = 50) {
  ...
  stdev <- 0.3 + x # spread increases with x
  y <- rnorm(n, 0, stdev) # error term has larger variance with x
  y <- compute.y(x, y, r)
  plot(
    x,
    y,
  ...
  x <- rnorm(n)
  x <- x + abs(min(x))
  a <- 0.3 + 1 * (max(x)) # calculate intercept of x-standard deviation function
  stdev <- a - x
  y <- rnorm(n, 0, stdev)
  y <- compute.y(x, y, r)
  plot(
